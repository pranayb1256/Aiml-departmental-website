...
const Academics = () => {
    ...
    const API_URL = import.meta.env.VITE_API_URL;
                ...
                const res = await axios.get(`${API_URL}/academics/calender`);
    ...
    const API_URL = import.meta.env.VITE_API_URL;
        ...
        setLoading(true);
        try {
            const { data } = await axios.get(`${API_URL}/result`);
            setResults(data);
        } catch (error) {
    ...
    const API_URL = import.meta.env.VITE_API_URL;
        ...
        try {
            const response = await axios.get(`${API_URL}/academics/timetable`);
    ...
    return (
        <div className="p-8 bg-gray-100 min-h-screen relative">
            ...
            <section className="my-12 bg-white p-8 shadow-lg rounded-xl">
                <motion.h2
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-xl font-semibold text-gray-800">
                            Project Competition Highlights
                        </h3>
                        <p className="text-gray-600">
                            <strong>Date Held:</strong> April 10, 2025
                        </p>
                        <p className="text-gray-700">
                            The annual AIML project competition was a remarkable display of talent and innovation. Students from various years and divisions—including 2nd year A & B divisions, 3rd year, final year, and the AIML stream—delivered outstanding projects across domains like healthcare, environment, and education.
                        </p>
                        <p className="text-gray-700">
                            Notably, the <span className="font-semibold text-blue-600">AIML departmental website</span> project, developed by 2nd year AIML students, earned 1st prize—an achievement that reflects the team's dedication, creativity, and technical skills.
                        </p>
                        <p className="text-gray-700">
                            Congratulations to all the winners and participants! Your achievements are a testament to the growing excellence within the AIML community. Keep up the inspiring work!
                        </p>
                    </div>

                    {/* Right - Image slider with visual distinction */}
                    <motion.div
                        className="w-full overflow-hidden rounded-xl border-l-4 border-blue-200 bg-blue-50 p-4 shadow-sm"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ duration: 0.7 }}
                    >
                        <h4 className="text-lg font-semibold text-blue-600 mb-4 text-center">
                            Mini-Project Winners
                        </h4>

                        <motion.div
                            className="flex gap-4"
                            animate={{ x: [`0%`, `-${100 * (images.length - 1)}%`] }}
                            transition={{
                                repeat: Infinity,
                                repeatType: "loop",
                                duration: 24,
                        ...
                        >
                            {images.map((filename, index) => (
                                <img
                                    key={index}
                                    src={`/proj_winners/${filename}`}
                                    alt={`Project ${index + 1}`}
                                    className="w-[400px] h-64 object-cover flex-shrink-0 rounded-xl"
                                />
                            ))}
                        </motion.div>
                    </motion.div>
